...
/**
 * Ant task for batch-compiling XML report design files. Works like the built-in
 * <code>javac</code> Ant task.
 * <p>
 ...
 * <li>xmlvalidation
 * </ul>
 * Of these arguments, the <code>src</code> and <code>destdir</code> are
 * required. When this task executes, it will recursively scan the
 * <code>src</code> and <code>destdir</code> looking for XML report design files
 * to compile. This task makes its compile decision based on timestamp and only
 * XML files that have no corresponding .jasper file or where the compiled
 * report design file is older than the XML file will be compiled.
 *
...
public class JRAntCompileTask extends JRBaseAntTask {
    ...
    /**
     * Sets the destination directory into which the XML report design files
     * should be compiled.
     *
    ...
    /**
     * Sets the temporary working directory into which to store the temporary
     * files generated during XML report design file compilation. This is only
     * used by the Java bytecode report compilers that need to have the Java
     * source files stored on disk in order to compile them.
     * <p>
     * If not set, the temporary working directory will be the current working
     * directory, as specified by the <code>user.dir</code> system property.
     *
     ...
     * to avoid deletion of the Java source files generated in the temporary
     * working directory during report generation. This is useful when
     * debugging.
     *
     * @param keepjava flag for preventing the deletion of generated Java source
     * files
     */
     ...
     * {@link net.sf.jasperreports.engine.design.JRCompiler} interface. When
     * specified, this value will temporarily override the value of the
     ...
     *
     * @return classpath to use when compiling the report associated Java
     * expressions class
     */
    ...
    /**
     * Instructs the XML parser to validate the XML report design file during
     * compilation.
     *
     * @param xmlvalidation flag for enabling/disabling the validation feature
     * of the XML parser
     */
    ...
    /**
     * Checks that all required attributes have been set and that the supplied
     * values are valid.
     */
...
//			}
     ...
     * are returned in the instance variable <code>reportFilesMap</code>.
     *
